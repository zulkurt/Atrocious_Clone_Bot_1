...
from bot.modules.helper_funcs.mirror_helpers.gd_downloader import add_gd_download
from bot.modules.helper_funcs.mirror_helpers.telegram_downloader import TelegramDownloadHelper
from bot.modules.helper_funcs.mirror_helpers.upload_status import UploadStatus
from bot.modules.helper_funcs.mirror_helpers.tg_upload_status import TgUploadStatus
from bot.modules.helper_funcs.mirror_helpers.gdriveTools import GoogleDriveHelper
from bot.modules.helper_funcs.mirror_helpers.pyrogramEngine import TgUploader
...
def _mirror(
    ...
    if reply_to is not None:
        ...
        if not is_url(link) or len(link) == 0:
    ...
    if not is_url(link) and not ospath.exists(link):
        help_msg = (f"Reply telegram file with /{BotCommands.MirrorCommand} to mirror in google drive")
    ...
    listener = MirrorListener(bot, message, pswd, tag)
    ...
    if multi > 1:
        ...
        Thread(
            target=_mirror,
            args=(bot, nextmsg, pswd, multi)).start()
