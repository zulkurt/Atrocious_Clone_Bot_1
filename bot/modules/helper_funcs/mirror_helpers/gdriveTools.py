...
from bot import GDRIVE_ID, DOWNLOAD_DIR, IS_TEAM_DRIVE, INDEX_URL, USE_SERVICE_ACCOUNTS, BUTTON_FOUR_NAME, BUTTON_FOUR_URL, BUTTON_FIVE_NAME, BUTTON_FIVE_URL, BUTTON_SIX_NAME, BUTTON_SIX_URL, VIEW_LINK, DRIVES_NAMES, DRIVES_IDS, INDEX_URLS, EXTENTION_FILTER
...
from bot.modules.helper_funcs.mirror_helpers.bot_utils import get_readable_file_size, setInterval
from bot.modules.helper_funcs.mirror_helpers.fs_utils import get_mime_type, get_path_size
...
class GoogleDriveHelper:
    ...
    def __upload_file(self, file_path, file_name, mime_type, GDRIVE_ID):
        ...
        if GDRIVE_ID is not None:
            file_metadata["parents"] = [GDRIVE_ID]
                ...
                if err.resp.get("content-type", "").startswith("application/json"):
                    ...
                    if USE_SERVICE_ACCOUNTS:
                        ...
                        return self.__upload_file(
                            file_path, file_name, mime_type, GDRIVE_ID
                        )
                    else:
    ...
    def clone(self, link, new_name):
        ...
        try:
            ...
            if mime_type == self.__G_DRIVE_DIR_MIME_TYPE:
                ...
                file = self.__copyFile(meta.get("id"), GDRIVE_ID, new_name)
    ...
    def __cloneFolder(self, name, local_path, folder_id, GDRIVE_ID):
        ...
        for file in files:
            if file.get("mimeType") == self.__G_DRIVE_DIR_MIME_TYPE:
                ...
                current_dir_id = self.__create_directory(file.get("name"), GDRIVE_ID)
                self.__cloneFolder(
                ...
                self.__copyFile(file.get("id"), GDRIVE_ID, file.get("name"))
            if self.is_cancelled:
                break
    ...
    def __create_directory(self, directory_name, GDRIVE_ID):
        file_metadata = {
            "name": directory_name,
            "description": "Uploaded by Mirror-leech-telegram-bot",
    ...
    def __upload_dir(self, input_directory, GDRIVE_ID):
        ...
        for item in list_dirs:
            ...
            if ospath.isdir(current_file_name):
                current_dir_id = self.__create_directory(item, GDRIVE_ID)
                ...
                self.__upload_file(current_file_name, file_name, mime_type, GDRIVE_ID)
                self.__total_files += 1
                new_id = GDRIVE_ID
            if self.is_cancelled:
                break
    ...
    def __drive_query(self, GDRIVE_ID, fileName, stopDup, isRecursive, itemType):
        try:
            if isRecursive:
                if stopDup:
                ...
                if stopDup:
                    query = f"'{GDRIVE_ID}' in parents and name = '{fileName}' and "
                else:
                    query = f"'{GDRIVE_ID}' in parents and "
                    fileName = fileName.split(" ")
                    for name in fileName:
                        if name != "":
    ...
    def drive_list(
        ...
        if len(DRIVES_IDS) > 1:
            ...
            if token_service is not None:
                self.__service = token_service
        for index, GDRIVE_ID in enumerate(DRIVES_IDS):
            if isRecursive and len(GDRIVE_ID) > 23:
                isRecur = False
            else:
                isRecur = isRecursive
            response = self.__drive_query(
                GDRIVE_ID, fileName, stopDup, isRecur, itemType
            )
                    ...
                    if INDEX_URLS[index] is not None:
                        if isRecur:
                            url_path = "/".join(
                                [
                                    rquote(n, safe="")
                                    for n in self.__get_recursive_list(file, GDRIVE_ID)
                                ]
                            )
                        else:
                            url_path = rquote(f'{file.get("name")}', safe="")
