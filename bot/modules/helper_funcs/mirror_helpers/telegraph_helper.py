...
class TelegraphHelper:
    ...
    def create_page(self, title, content):
        try:
           return self.telegraph.create_page(title = title, author_name=self.author_name, author_url=self.author_url, html_content=content)
    ...
    def edit_page(self, path, title, content):
        try:
            return self.telegraph.edit_page(path = path, title = title, author_name=self.author_name, author_url=self.author_url, html_content=content)
        except RetryAfterError as st:
            LOGGER.warning(f'Telegraph Flood control exceeded. I will sleep for {st.retry_after} seconds.')
            sleep(st.retry_after)
        ...
        for content in telegraph_content :
            if nxt_page == 1 :
                ...
                if nxt_page < num_of_path:
                    content += f'<b> | <a href="https://telegra.ph/{path[nxt_page]}">Next</a></b>'
                    nxt_page += 1
            self.edit_page(path = path[prev_page], title = 'Atrocious Mirror', content=content)
        return


telegraph=TelegraphHelper('Atrocious Mirror', 'https://t.me/+Xwwi7toV4YsyMWJl')
