...
from bot import LOGGER, UNIFIED_EMAIL, UNIFIED_PASS, GDTOT_CRYPT, HUBDRIVE_CRYPT, KATDRIVE_CRYPT, DRIVEFIRE_CRYPT, XSRF_TOKEN, laravel_session, SHAREDRIVE_PHPCKS, config_dict
...
def gdtot(url: str) -> str:
    ...
    url = re_findall(r'URL=(.*?)"', res.text)[0]
    info = {}
    info["error"] = False
...
def unified(url: str) -> str:
    ...
    client = create_scraper(delay=10, browser='chrome')
...
def sharer_pw_dl(url: str)-> str:
    ...
    res = client.get(url)
    token = re_findall("_token\s=\s'(.*?)'", res.text, re.DOTALL)[0]
